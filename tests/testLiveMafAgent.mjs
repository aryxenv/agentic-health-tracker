...
test("Live MAF Health Agent End-to-End Suite with Groq gpt-oss-120b", async (t) => {
  ...
  await t.test(
    "Scenario 1: Food logging with USDA FoodData Central tool calls",
    async () => {
      const steps = [];
      const messages = [
        {
          role: "user",
          content:
            "I had 150g grilled chicken breast and 1 cup white rice for lunch",
        },
      ];

      console.log("\n--- Running Scenario 1: Food Logging ---");
      const result = await runHealthAgentStream(
        messages,
        testProfile,
        (step) => {
          steps.push(step);
          console.log(
            `  [Agentic Step: ${step.type.toUpperCase()}] ${step.title}`,
          );
        },
      );
      ...
      console.log(
        "Draft Entries:",
        JSON.stringify(result.draft_entries, null, 2),
      );

      assert.ok(result.reply, "Result must have reply");
      assert.ok(
        Array.isArray(result.draft_entries),
        "Result must have draft_entries",
      );
      assert.ok(
        result.draft_entries.length >= 1,
        "Should have produced at least 1 food entry",
      );
      ...
      assert.ok(
        ...
        "Agent should have executed tool calls",
      );
      console.log(
        `  Total agentic steps emitted: ${steps.length} (${toolCallSteps.length} tool calls)`,
      );
    },
  );

  await t.test(
    "Scenario 2: Physical Activity logging with 2024 Adult Compendium MET tool calls",
    async () => {
      const steps = [];
      const messages = [
      ...
      const activityEntry = result.draft_entries.find(
        (e) => e.type === "activity",
      );
      ...
      assert.ok(
        activityEntry.durationMin >= 40,
        "Duration should reflect ~45 mins",
      );
    },
  );

  await t.test(
    "Scenario 3: Multi-Turn Context Resolution across stateless turns",
    async () => {
      const steps = [];
      ...
      const messages = [
        { role: "user", content: "I ate 2 boiled eggs for breakfast" },
        {
          role: "assistant",
          content:
            "Logged 2 boiled eggs (~144 kcal, 12.6g protein, 0.8g carbs, 9.6g fat). Would you like to confirm this entry?",
        },
        {
          role: "user",
      ...
      assert.ok(
        eggEntry.calories >= 180,
        `Egg calories should reflect 3 eggs (~216 kcal), got ${eggEntry.calories}`,
      );
      ...
      assert.ok(
        bananaEntry,
        "Should identify added banana in multi-turn request",
      );
    },
  );
});
